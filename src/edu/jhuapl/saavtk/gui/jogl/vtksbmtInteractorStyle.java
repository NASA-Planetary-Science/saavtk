...
class vtksbmtInteractorStyle extends vtkInteractorStyle implements MouseListener
{
    ...
    vtksbmtJoglComponent vtkComponent; // unfortunately it is necessary to keep a reference to the canvas itself so
                                       // that mouse coordinates can be computed appropriately
    vtkRenderWindowInteractor interactor;
    Map<Integer, StyleEvent> eventObserverTags = Maps.newConcurrentMap(); // this keeps track of the (int) id of the observer assigned to listen for each
                                                                          // respective event, so one could delete it (by id) later if necessary
    ...
    public void OnMouseWheelForward()
    {
        ...
        storeMousePosition(); // need to call this to commit further actions to the correct renderer (via
                              // FindPokedRenderer)
...
//        System.out.println("Rotate");
        ...
        double delev = -20. / size[1];
        double dazim = -20. / size[0];
...
//        System.out.println("Dolly");
    ...
    //
    // These overrides are for MouseListener which this class implements, but we set
    // them as deprecated (except for mouseClicked which handles multiple clicks) to
    // warn future developers not to use them (so they don't interfere with the
    // vtkInteractorStyle methods OnLeftButtonPressed, etc.)
    //
    ...
    public void OnLeftButtonSingleClick()
    {
        ...
        if (cellPicker.GetCellId() != -1)
        {
            ...
            /*
             * vtkTextActor textActor=new vtkTextActor(); vtkTextMapper textMapper=new
             * vtkTextMapper(); textActor.SetMapper(textMapper);
             * textMapper.SetInput("Test");
             * textActor.SetPosition(cellPicker.GetSelectionPoint());
             * textActor.SetVisibility(1); canvas.getRenderer().AddActor(textActor);
             */
            /*
             * vtkTextActor actor=new vtkTextActor(); actor.SetInput("Test");
             * actor.SetPosition(0,0); actor.GetTextProperty().SetFontSize(24);
             * actor.GetTextProperty().SetColor(1,1,0);
             * canvas.getRenderer().AddActor2D(actor);
             */
            /*
             * vtkTextActor actor=new vtkTextActor(); actor.SetInput("Test");
             * actor.GetTextProperty().SetColor(0,1,0); vtkTextWidget widget=new
             * vtkTextWidget(); vtkTextRepresentation representation=new
             ...
             * widget.SetInteractor(this.GetInteractor()); widget.SetTextActor(actor);
             * widget.SelectableOff(); widget.On();
             */
            /*
             * vtkVectorText text=new vtkVectorText(); text.SetText("Test");
             * vtkPolyDataMapper mapper=new vtkPolyDataMapper();
             * mapper.SetInputConnection(text.GetOutputPort()); mapper.Update(); vtkFollower
             * follower=new vtkFollower(); follower.SetMapper(mapper);
             * follower.GetProperty().SetColor(0,1,0);
             * follower.SetCamera(canvas.getActiveCamera());
             * follower.SetOrigin(worldPosition); double[] bounds=follower.GetBounds();
             * System.out.println(bounds[0]+" "+bounds[1]+" "+bounds[2]+" "+bounds[3]+" "
             * +bounds[4]+" "+bounds[5]); canvas.getRenderer().AddActor(follower);
             */
        }
    }

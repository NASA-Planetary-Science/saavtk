...
//			}
    ...
    public vtkPolyData GetOutput()
    {
        ...
        /*
         * vtkPoints points = new vtkPoints(); vtkCellArray cells = new vtkCellArray();
         * // Stopwatch sw=new Stopwatch(); // sw.start(); for (int i = 0; i <
         * data.size(); i++) { // if (sw.elapsedMillis()>4000) // { // sw.reset(); //
         * sw.start(); // System.out.println(i+"/"+data.size()); // } double[] tuple =
         * data.get(i); double[] pos = new double[] { tuple[0], tuple[1], tuple[2] };
         * int id = points.InsertNextPoint(pos); vtkVertex vert = new vtkVertex();
         * vert.GetPointIds().SetId(0, id); cells.InsertNextCell(vert); pos = null; }
         * vtkPolyData polyData = new vtkPolyData(); polyData.SetPoints(points);
         * polyData.SetVerts(cells); return polyData;
         */
    }

...
//			ls.add(s);
    ...
    /*
     * public static Collection<PatchStructure> readPatchStructures(Path shapeFile)
     * throws IOException { Collection<SimpleFeature> sfc = read(shapeFile);
     * Set<PatchStructure> ps = Sets.newHashSet(); for (SimpleFeature sf : sfc)
     * ps.add(FeatureUtil.createPatchStructureFrom(sf)); return ps; }
     */
    ...
    /*
     * public static void writePatchStructures(Collection<PatchStructure> psc, Path
     * shapeFile) throws IOException { List<SimpleFeature> fl =
     * Lists.newArrayList(); for (PatchStructure ps : psc) { SimpleFeature f =
     * FeatureUtil.createFeatureFrom(ps); fl.add(f); } ShapefileUtil.write(fl,
     * shapeFile); }
     */
...
//				System.out.println(ls);
             ...
             * Paths.get("/Users/zimmemi1/Desktop/shape/myPatches.shp");
             * ShapefileUtil.writePatchStructures(patches, patchesShapeFile);
             * Collection<PatchStructure> patchesRead =
             * ShapefileUtil.readPatchStructures(patchesShapeFile); for (PatchStructure ps :
             * patchesRead) System.out.println(ps);
             */
    ...
    public static void test2()
    {
        ...
        /*
         * List<PatchStructure> patches=Lists.newArrayList(); for (int i=0;
         * i<polyData.GetNumberOfCells(); i++) { vtkPoints
         * triPoints=polyData.GetCell(i).GetPoints(); double[][] pts=new
         * double[triPoints.GetNumberOfPoints()][]; for (int j=0;
         * j<triPoints.GetNumberOfPoints(); j++) pts[j]=triPoints.GetPoint(j);

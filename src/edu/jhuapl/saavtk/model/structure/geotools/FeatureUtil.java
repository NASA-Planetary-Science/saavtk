...
//	private static SimpleFeatureBuilder	patchBuilder	= new SimpleFeatureBuilder(patchType);
	...
	public static SimpleFeature createFeatureFrom(PointStructure ps)
	{
		...
		Geometry geom = JTSFactoryFinder.getGeometryFactory()
				.createPoint(new Coordinate(location[0] * 1000, location[1] * 1000, location[2] * 1000));
...
/*	public static SimpleFeature createPointFeatureFrom(List<Vector3D> points)
	...
	public static SimpleFeature createFeatureFrom(LineStructure ls)
	{
		...
		for (int i = 0; i < ls.getNumberOfSegments(); i++)
		{
			...
			Coordinate[] coords = new Coordinate[2];
			coords[0] = new Coordinate(s.getStart()[0] * 1000, s.getStart()[1] * 1000, s.getStart()[2] * 1000);
			coords[1] = new Coordinate(s.getEnd()[0] * 1000, s.getEnd()[1] * 1000, s.getEnd()[2] * 1000);
			// the coordinates are now in the DefaultGeocentric.CARTESIAN CRS
	...
	public static PointStructure createPointStructureFrom(SimpleFeature feature)
	{
		...
		try
		{
			MathTransform transform = CRS.findMathTransform(feature.getType().getCoordinateReferenceSystem(),
	...
	public static LineStructure createLineStructureFrom(SimpleFeature feature) 
	{
		...
		try
		{
			transform = CRS.findMathTransform(feature.getType().getCoordinateReferenceSystem(),
					DefaultGeocentricCRS.CARTESIAN, true);
	...
	protected static List<LineSegment> parse(MultiPolygon mp, MathTransform transform) throws TransformException
	{
		List<LineSegment> segments = Lists.newArrayList();
	...
	protected static List<LineSegment> parse(MultiLineString mls, MathTransform transform) throws TransformException
	{
	...
	protected static List<LineSegment> convertCoordsToSegments(Coordinate[] coords, MathTransform transform,
			boolean closed) throws TransformException
	{
		List<LineSegment> segments = Lists.newArrayList();
	...
	public static double[][] pointsFromCoordinates(Coordinate[] coords)
	{

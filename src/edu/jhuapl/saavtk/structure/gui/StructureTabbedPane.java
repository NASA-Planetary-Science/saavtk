...
import edu.jhuapl.saavtk.pick.RoundStructurePicker;
import edu.jhuapl.saavtk.structure.StructureManager;

/**
 * UI component used to display a list of 5 child {@link StructurePanel}s. These 5 (line, polygon, circle, ellipse,
 * points) panels allow the user to manipulate the structures associated with the panel.
 *
 * @author lopeznr1
 */
public class StructureTabbedPane extends JTabbedPane
{
	...
	/**
	 * Helper method to form the proper {@link StructurePanel} for the specified arguments.
	 *
	 * @param aModelNames
	 */
	private StructurePanel<?> formStructurePanel(ModelNames aModelNames)
	{
		...
		refPickManager.getDefaultPicker().addListener((PickListener) tmpStructureManager);
		...
		else if (aModelNames == ModelNames.CIRCLE_STRUCTURES || aModelNames == ModelNames.ELLIPSE_STRUCTURES
				|| aModelNames == ModelNames.POINT_STRUCTURES)
		{
			var tmpEllipseManager = (AbstractEllipsePolygonModel) tmpStructureManager;
			var tmpMode = tmpEllipseManager.getMode();
			tmpPicker = new RoundStructurePicker(refRenderer, tmpSmallBody, tmpEllipseManager, tmpMode);
		}
		else

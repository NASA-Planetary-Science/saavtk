...
public class CylindricalProjection implements Projection
{
	...
	public vtkPolyData clipVisibleGeometry(vtkPolyData polyData)
	{
		...
		{
			vtkImplicitFunction maxFunction=PolyDataClipWithCone.generateConeFunction(Vector3D.ZERO, Vector3D.PLUS_K, 90-latMaxDeg);
			...
			//vtkPolyData minPolyData=new PolyDataClip(minFunction).apply(maxPolyData);
			vtkImplicitFunction minFunction=PolyDataClipWithCone.generateConeFunction(Vector3D.ZERO, Vector3D.PLUS_K, 90-latMinDeg);
		...
		{
			return clipLatitude(latMinDeg, latMaxDeg, clipLongitude(lonMinDeg, lonMaxDeg, polyData));	
		}
	//	
...
/*	public static vtkPolyData clipBelowLatitude(double latDeg, vtkPolyData polyData)
	{
		if (latDeg>0)
	...
	public static vtkPolyData clipAboveLatitude(double latDeg, vtkPolyData polyData)
	{
		if (latDeg<0)
	...
	public static vtkImplicitFunction createDoubleConeFunction(double latMinDeg, double latMaxDeg)
	{
		...
		maxFunction=PolyDataClipWithCone.generateConeFunction(Vector3D.ZERO, Vector3D.PLUS_K, latMaxDeg>0?90-latMaxDeg:90+latMaxDeg);
		
/*		if (latMinDeg>0)
			minFunction=PolyDataClipWithCone.generateClipFunction(Vector3D.ZERO, Vector3D.PLUS_K, 90-latMinDeg);
		else
			minFunction=PolyDataClipWithCone.generateClipFunction(Vector3D.ZERO, Vector3D.MINUS_K, 90+latMinDeg);*/
		
		//bool.AddFunction(minFunction);
		
		//if (latMinDeg*latMaxDeg>0)	// e.g. if both are negative or both are positive
		//{
	...
	public static vtkImplicitFunction createWedgeFunction(double lonMinDeg, double lonMaxDeg)
	{
		...
		Plane minPlane=new Plane(Vector3D.ZERO, Vector3D.PLUS_J);	// setting the normal to +y aligns the plane with the prime meridian along +x at longitude 0
		minPlane=minPlane.rotate(Vector3D.ZERO, new Rotation(Vector3D.PLUS_K, Math.toRadians(lonMinDeg)));

		Plane maxPlane=new Plane(Vector3D.ZERO, Vector3D.MINUS_J);	// note the change in normal direction here, to invert the "front" of the plane
...
/*		return new vtkImplicitFunction(){
			...
			public double EvaluateFunction(double id0, double id1, double id2) {
				double thDeg=Math.toDegrees(Math.atan(id1/id0));
				if (thDeg>=lonMinDeg && thDeg<=lonMaxDeg)
					return -1;
				else
					return 1;
			}
			...
			public double EvaluateFunction(double[] id0) {
				return this.EvaluateFunction(id0[0], id0[2], id0[2]);
			}

			@Override
			public double FunctionValue(double id0, double id1, double id2) {
				return this.EvaluateFunction(id0, id1, id2);
			}
			
			@Override
			public double FunctionValue(double[] id0) {
				return this.EvaluateFunction(id0);
			}
		};*/
	}
...
/*	public static vtkPolyData clipAboveLongitude(double lonDeg, vtkPolyData polyData)
	{
	...
	public static vtkPolyData clipBelowLongitude(double lonDeg, vtkPolyData polyData)
	{
	...
	public static List<vtkActor2D> generateLatitudeLabels(int nLatHemi, double lonDeg, GenericPolyhedralModel model, boolean labelPoles)
	{
		String fmt = "%.0f";
		String degSymbol = String.valueOf(Character.toChars(0x00B0));
		...
		List<vtkActor2D> actors=Lists.newArrayList();
		double[] latRange=LinearSpace.create(0,90, nLatHemi);
		for (int i = labelPoles?0:1; i < nLatHemi; i++) // Start at one to skip labeling the pole.
		{
		...
		// southern hemisphere
		for (int i = labelPoles?0:1; i < nLatHemi-1; i++) // Start at one to skip labeling the pole.
		{
			// Compute labels from the poles toward the equator so they coincide with parallels from the grid
			// no matter what spacing.
			double lat = Math.toRadians(90 - latRange[i]);

...
//            System.arraycopy(result, 0, vectorOut, 0, vectorOut.length);
    ...
    public static double[] convertToDoubles(String[] cells) {
        double[] values = new double[cells.length];
        for (int i = 0; i < values.length; i++) 
        {
            values[i] = Double.parseDouble(cells[i].replace("\"", ""));
        }
        return values;
    }

    public static int[] convertToInts(String[] cells) {
        int[] values = new int[cells.length];
        for (int i = 0; i < values.length; i++) {
            values[i] = Integer.parseInt(cells[i].replace("\"", ""));
        }
        return values;
    }

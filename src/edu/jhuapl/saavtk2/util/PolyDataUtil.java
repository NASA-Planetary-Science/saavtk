...
public class PolyDataUtil
{
    ...
    public static double[] rayIntersect(vtksbCellLocator cellLocator, double[] origin, double[] endpoint)
    {
        double tol = 1e-6;
        ...
        int result = cellLocator.IntersectWithLine(origin, endpoint, tol, t, x, pcoords, subId, cellId, cell);
    ...
    public static vtkPolyData computeMultipleCylindricalProjectionIntersection(
            vtkPolyData polyData,
            vtksbCellLocator locator,
            vtkPointLocator pointLocator,
            List<CylindricalProjection> projections)
    {
    	CylindricalProjection projection=projections.get(0);
    	polyData=projection.clipVisibleGeometry(polyData);
    	for (int i=1; i<projections.size(); i++)
    	{
    		if (polyData==null || polyData.GetNumberOfPoints()==0 || polyData.GetNumberOfCells()==0)
    			return null;
    		
    		polyData=projections.get(i).clipVisibleGeometry(polyData);
    	}
    	return polyData;
    }
            ...
            List<Frustum> frustums)
    {
        ...
        polyData = computeFrustumIntersection(polyData, locator, pointLocator, f);
            ...
            Frustum f)
    {
        ...
        double[] origin=f.getOrigin().toArray();
        double[] ul=f.getUpperLeftUnit().toArray();
        double[] ur=f.getUpperRightUnit().toArray();
        double[] lr=f.getLowerRightUnit().toArray();
        double[] ll=f.getLowerLeftUnit().toArray();
        
/*        Vector3D f1=f.getLowerLeft();
        Vector3D f2=f.getLowerRight();
        Vector3D f3=f.getUpperRight();
        Vector3D f4=f.getUpperLeft();
        System.out.println(f.getOrigin().getX()+" "+f.getOrigin().getY()+" "+f.getOrigin().getZ());
        System.out.println(f1.getX()+" "+f1.getY()+" "+f1.getZ());
        System.out.println(f2.getX()+" "+f2.getY()+" "+f2.getZ());
        System.out.println(f3.getX()+" "+f3.getY()+" "+f3.getZ());
        System.out.println(f4.getX()+" "+f4.getY()+" "+f4.getZ());
  */      
/*        Vector3D center=f1.add(f2.add(f3.add(f4))).normalize();
        Rotation rot=new Rotation(center, Vector3D.PLUS_K);
        System.out.println(rot.applyTo(center));
        System.out.println(rot.applyTo(f1));
        System.out.println(rot.applyTo(f2));
        System.out.println(rot.applyTo(f3));
        System.out.println(rot.applyTo(f4));*/
...
//            frustum.computeTextureCoordinatesFromPoint(pt, uv, false);
            Frustum.adjustTextureCoordinates(1, 1, uv);	// XXX: not sure this does anything
            if (uv[0]>=0 && uv[0]<=1 && uv[1]>=0 && uv[1]<=1)
                overlap+=1;
        }
        return overlap/(double)nPoints;
    }
...
//            System.out.println("Make sure faces have correct orientation and are connected without holes.");
    ...
    static public void generateTextureCoordinates(Frustum frustum, int width, int height, vtkPolyData footprint)
    {
        ...
        double[] spacecraftPosition = frustum.getOrigin().toArray();
        double[] frustum1 = frustum.getUpperLeftUnit().toArray();
        double[] frustum3 = frustum.getUpperRightUnit().toArray();
        double[] frustum2 = frustum.getLowerLeftUnit().toArray();
    ...
    public static void generateTextureCoordinates(CylindricalProjection projection, int width, int height, vtkPolyData footprint)
    {
        int numberOfPoints = footprint.GetNumberOfPoints();
        vtkPointData pointData = footprint.GetPointData();
        vtkDataArray textureCoordinates = pointData.GetTCoords();
        ...
        {
            double[] pt = points.GetPoint(i);
            MapCoordinates mCoords=projection.project(new Vector3D(pt));
			double tx=(mCoords.getX()-projection.getHorizontalMin())/projection.getHorizontalExtent();
			double ty=(mCoords.getY()-projection.getVerticalMin())/projection.getVerticalExtent();
			textureCoordinates.SetTuple2(i, tx, ty);
        }
